...
Custom Drupal 11 theme (generated from `starterkit_theme`) for the Flavourful
...
ddev exec 'cd web/themes/custom/flavourful && npm install'

# One-off compile (compressed, for committing):
ddev exec 'cd web/themes/custom/flavourful && npm run build'

# Expanded build with source maps:
ddev exec 'cd web/themes/custom/flavourful && npm run dev'

# Watch and recompile on change while theming:
ddev exec 'cd web/themes/custom/flavourful && npm run watch'
```
...
- **`flavourful/global-styling`** — `css/global.css`. Attached site-wide via the
  `libraries:` key in `flavourful.info.yml`.
- **`flavourful/recipes`** — `css/recipes.css`. Attached on demand from
  `templates/views/views-view--recipes.html.twig` with
  `{{ attach_library('flavourful/recipes') }}` so it only loads where recipe
  cards render.
