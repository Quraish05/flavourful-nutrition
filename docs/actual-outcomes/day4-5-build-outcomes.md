...
| 3 | **Coding-standards nits in the shipped block.** `build()` in [`NutritionFactsBlock.php`](../../docroot/modules/custom/flavorful_nutrition/src/Plugin/Block/NutritionFactsBlock.php) is not indented inside the class body, and the new `create()` argument line has a stray leading space. | Hand-editing the method against the lab snippet; `phpcs` wasn't run before staging. | Behaviour is correct; formatting only. Run `ddev exec phpcbf --standard=Drupal` on the module before the commit that ships it. |
