...
**4. Listing wired through the teaser template, not `views-view-unformatted`.** The Day 8 baseline row template — now [`partials/legacy-recipes-row.html.twig`](../../docroot/themes/custom/flavourful/templates/partials/legacy-recipes-row.html.twig) — is not what renders the listing: the recipes view uses **grid** style with an **`entity:node`** row in **teaser** view mode, so rows render through `node--recipe--teaser.html.twig`. Putting the embed there is what makes the card appear per row — and is a cleaner "one component, many contexts" than a bespoke row template. It was renamed out of the `views-view-unformatted--recipes` suggestion because a file with that name is registered in the theme registry whether or not any view reaches it; under `partials/` it matches no theme hook and cannot be selected.
